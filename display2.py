...
class Model():
    ...
    def create_image(self, size):
        ...
        self.image = pil_image.resize(
            (int(ratio * pil_image.width), int(ratio * pil_image.height)))
        t2 = time.time()
...
class View():
    ...
    def create_widgets(self):
        ...
        # テロップを配置するフレームの作成と配置
        self.telop_frame = tkinter.Frame(self.main_frame)
        self.telop_frame.grid(column=1, row=2)

        # テロップ移動ボタンのフレームの作成と配置
        self.telop_button_frame = tkinter.Frame(self.main_frame)
        self.telop_button_frame.grid(column=1, row=3)
        ...
        # ファイル読み込みボタンの作成と配置
        self.text_first_step = tkinter.Label(self.operation_frame,
                                             text="手順１：動画ファイルを選択する")
        self.text_first_step.pack()
        self.text_first_step_path = tkinter.Label(self.operation_frame,
                                                  text="動画パス")
        self.text_first_step_path.pack()
        self.load_button = tkinter.Button(self.operation_frame, text="動画選択")
        self.load_button.pack()

        # 文字起こし実行読み込みボタンの作成と配置
        self.text_second_step = tkinter.Label(self.operation_frame,
                                              text="手順2：自動文字起こしをする")
        self.text_second_step.pack()
        self.whisper_button = tkinter.Button(self.operation_frame,
                                             text="自動文字起こしを実行する")
        self.whisper_button.pack()

        # テロップ修正指示のテキストの作成と配置
        self.text_third_step = tkinter.Label(self.operation_frame,
                                             text="手順3：テロップを確認＆修正する")
        self.text_third_step.pack()
        self.load_telop = tkinter.Button(self.operation_frame,
                                         text=u'テロップを読み込む')
        self.load_telop.pack()
        self.write_telop = tkinter.Button(self.operation_frame,
                                          text=u'テロップを保存する')
        self.load_telop.pack()

        # 動画作成ボタンの作成と配置
        self.text_forth_step = tkinter.Label(self.operation_frame,
                                             text="手順4：動画を出力する")
        self.text_forth_step.pack()
        self.text_forth_step_comment = tkinter.Label(
            self.operation_frame, text="動画は元動画と同じディレクトリに出力されます")
        self.text_forth_step_comment.pack()
        self.create_movie_button = tkinter.Button(self.operation_frame,
                                                  text="動画を出力する")
        self.create_movie_button.pack()

        # テロップ修正用のテキストボックスの作成と配置
        self.telop_canvas = tkinter.Text(self.telop_frame, height=10)
        self.telop_canvas.pack()

        self.back_telop = tkinter.Button(self.telop_button_frame,
                                         text=u'前のテロップへ')
        self.back_telop.grid(column=1, row=1)

        self.next_telop = tkinter.Button(self.telop_button_frame,
                                         text=u'次のテロップへ')
        self.next_telop.grid(column=2, row=1)
...
class Controller():
    ...
    def set_events(self):
        ...
        self.view.canvas.bind("<ButtonPress-1>", self.button_press)
    ...
    def draw(self):
        ...
        if self.playing:
            # フレームの画像を作成
            self.model.create_image((self.view.canvas.winfo_width(),
